...
from paper_record.VAEMTL.trans_padding import Conv1d as conv1d
...
class CNN_Fusion(nn.Module):
    def __init__(self, args):
        ...
        '''卷积'''
        # 4卷积
        self.convs4_2 = nn.Sequential(nn.Conv1d(self.sequence_out, 768, 2),
        ...
        #两卷积
        self.l2_pool = 768
        self.convs2_1 = nn.Sequential(conv1d(self.l2_pool, 768, 3),
        ...
        self.self_text_att_average = nn.Sequential(
            nn.Linear(32, self.final_hid), nn.BatchNorm1d(111), nn.ReLU())
        # 均值后数句
        self.self_text_re_Dro = nn.Sequential(nn.Linear(32, 32),
                                              ...
                                              nn.Dropout(self.drop_rate))
        # 层标准化
        self.self_text_layer_norm = nn.LayerNorm([111, 32])
        # flatten_self
        self.self_text_faltten = nn.Sequential(nn.Linear(111 * 32, 32),
                                               nn.BatchNorm1d(32), nn.ReLU())
        # image
        self.multi = nn.Linear(768, 32)
        self.multi2 = nn.Linear(111, 49)
        self.merge = nn.Linear(1024, 32)
        self.mcb = CompactBilinearPooling(49, 49, 32)
        # 融合激活
    ...
    def decode(self, z, max_len):
        recon_text = self.text_decoder(z, max_len)  #32 87
    ...
    def forward(self, text, image, mask):
        ...
        sequence_output = torch.cat(
            (sequence_output_1, sequence_output_2, sequence_output_3,
             sequence_output_4, sequence_output_5), 2)
        ...
        multi_text = self.multi2(multi_text)
        '''双流2''' '''两个全连接--32维度-便于合并'''
        text = torch.flatten(l2_pool, start_dim=1, end_dim=2)  # end_dim=-1
        merge = []
        ...
        multi_image = Im.permute(
            0, 2, 1
        )  # b, w, h, c) should work. Just make sure that “c” is last. That’s because the FFT works on the last dimension.
        ...
        Merge_feture = self.merge(Merge_feture)  # 32 32
        merge.append(Merge_feture)  # 32
        ...
        for layer in range(att_layers):
            ...
            for _ in range(head):
                ...
                '''将注意力均值化-无法平均-也就是多个列表，对应的元素相加求平均在进行成为当前元素'''
                self_text_att_img21 = self.self_text_att_average(
                    self_text_att_img)
                self_text_att_img22 = self.self_text_att_average(
                    self_text_att_img)
                self_text_att_img23 = self.self_text_att_average(
                    self_text_att_img)
                self_text_att_img24 = self.self_text_att_average(
                    self_text_att_img)
                # 均值
                self_text_att_img2 = self_text_att_img21.add(
                    self_text_att_img22).add(self_text_att_img23).add(
                        self_text_att_img24)
                self_text_att_img2 = torch.div(self_text_att_img2, 4)
                '''均值后数据'''
                print(self_text_att_img2.shape)  # 32 111 32
                self_text_att_img2 = self.self_text_re_Dro(self_text_att_img2)
                '''将注意力后的数据相加:image_+self_att_img'''
                self_text_att_img = torch.add(self_text_query,
                                              self_text_att_img2)
                # print(self_att_img.shape) # [32,49,32]
                '''层标准化'''
                self_text_att_img = self.self_text_layer_norm(
                    self_text_att_img)
                '''不能将他直接merge进去因为，维度不同，faltten之后进行输入'''
        ...
        '''将后面的两维度flatten，变成二维'''
        self_text_att_img = torch.flatten(self_text_att_img,
                                          start_dim=1,
                                          end_dim=2)  # end_dim=-1
        ...
        # print(z.shape)
        # 重建的text和图像
        recon_text, recon_img = self.decode(z, text_decoder.shape[1])  # 32 87
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels,
            ...
            class_cost_vector.append(loss.item())
...
if __name__ == '__main__':
    ...
    train = ''
    test = ''
    output = '/tmp/pycharm_project_815/src/MBPAM_decoder_weight/'
