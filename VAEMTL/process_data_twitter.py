...
def read_image():
    ...
    file_list = [
        'Data/twitter/images_train/',
        'Data/twitter/images_validation/',
...
def get_text_dict(flag):
    ...
    #event label 都是由singlepass进行汇聚出来的，后期也要进行可视化处理，生成词云
    if flag == 'train':
...
def write_data(flag, image, text_only):
    ...
    def read_post(flag):
        ...
        if flag == "train":
            ...
            texts = open(pre_path + 'train_posts.txt', 'r').readlines()
            #train里面的数据进行了分开,里面有train——validate
            #根据validate取到图像数据-跟train不一样的数据
            images = [
                image.split('.')[0]
        ...
        for i, line in enumerate(texts):
            if i == 0: continue
            line_data = []
            if flag == 'train' or flag == 'validate':  #train和validate公用一个
                image_id = line.split('\t')[3].lower().split(',')
            else:
                image_id = line.split('\t')[4].lower().split(',')
            continue_flag = False
            for image in image_id:  #文本中切分出来的2
                if image in images:  #image是从图像标签切分3
                    image_id = image  #如果都在的交集，
                    continue_flag = True
                    break
            ...
            post_id = line.split('\t')[0]  #从train中找出id
            text = text_dict[post_id]  #根据id找到翻译后的数据

            if len(text.split(' ')) <= 2:
                # print(text)
                continue
            label = 0 if line.split(
                '\t')[-1].strip() == 'real' else 1  #根据元数据得到label
            event_name = re.sub(u'fake', '', image_id)
            event_name = re.sub(u'real', '', event_name)
            event_name = re.sub(u'[0-9_]', '', event_name)  #得到event
            #波士顿
            if event_name not in map_id:  #增加event
                map_id[event_name] = len(map_id)
                #一开始map_ID为空，则，第一个就是1，map{boston:0},下一个map{sandy:1}
                event = map_id[event_name]
            else:
                event = map_id[event_name]  #如果已经有这个event数据，找到这个对应的id
            line_data.append(post_id)
            line_data.append(image_id)
            post_content.append(text)
            line_data.append(text)  #翻译后的数据
            line_data.append([])
    ...
    post_content, post, event_num = read_post(flag)  #根据validate 取到
    print(flag + "大小：" + str(len(post_content)))
    print(flag + "维度：" + str(post.shape))
    print(flag + "事件分类器类别是：" + str(event_num))
    ...
    def paired(text_only=False):
        ...
        print(flag + "数据集" + "Label number is " + str(len(label)),
              "Rummor number is " + str(sum(label)),
              "Non rummor is " + str(len(label) - sum(label)))
    ...
    print("paired post length is " + str(len(paired_data["post_text"])),
          "paried data has " + str(len(paired_data)) + " dimension")
...
def get_data(text_only):
    ...
    if text_only:
        ...
        image_list = read_image()  #拿到数据照片：train，val，test
    print("-" * 50, "加载训练集", "-" * 50)
